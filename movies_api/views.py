...
def MovieGenres(request,genre):
    #print(genre)
    query_from = int(request.GET.get('from')) if request.GET.get('from') else 1800
    ...
    if genre == 'All':
        for g in ALL_GENRES:
            ...
            movies[g] = Movies.objects.filter(release_date__range=(year_from, year_to)).filter(genres__contains=g).count()

    elif genre == 'Rating':
        query_genre = request.GET.get('genre') if request.GET.get('genre') else ''
        year_from = str(query_from) + "-01-01"
        year_to = str(query_to) + "-01-01"
        movies_data = Movies.objects.filter(release_date__range=(year_from, year_to)).filter(genres__contains=query_genre)
        ...
        for y in range(int(query_from), int(query_to)):
            ...
            movies[y] = Movies.objects.filter(release_date__range=(year_from, year_to)).filter(genres__contains=genre).count()
